...
HALO는 이 세 실패를 동시에 해결하는 **4-Layer 온디바이스 아키텍처**다. KT Mi:dm 2.0 Mini(2.3B)를 주력 온디바이스 sLLM으로, Upstage Solar-pro를 클라우드 보조로 활용하여 **국내 AI 연계 트랙 요건을 충족**한다. 핵심 차별점은 본 팀장이 Google DeepMind × Kaggle AGI Benchmark Challenge 2026에 제출한 연구 **MetaMirage**(부호 역전 발견, r = −0.84, n = 7)를 **사용자 메타인지 추적에 전이**하는 Layer 3 설계다.
...
└───────────────────┬────────────────────────────────────┘
                    ↓
┌────────────────────────────────────────────────────────┐
│ Layer 3: METACOGNITION PROBE — MetaMirage 전이          │
...
### 3.5 Layer 3 — METACOGNITION PROBE
